...
if ($_SERVER["REQUEST_METHOD"] == "POST" 
    ...
    // Verificar disponibilidad: que no haya otra cita en la misma fecha y hora
    if (empty($errores)) {
    ...
    // ---------------- PROCESAR UPDATE ----------------
    if (empty($errores)) {
        $sql = $conexion->prepare("UPDATE citas SET fecha=:fecha, hora=:hora, servicio_id=:id_servicio  WHERE id=:id_cita AND usuario_id=:id_usuario");
